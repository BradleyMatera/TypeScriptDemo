    ...
    if (gameContainer && attemptsContainer && scoreContainer && statusContainer) {
        ...
        gameState.cards.forEach((card)=>{
            ...
            cardElement.addEventListener("click", ()=>{
                ...
                if (gameState.status === (0, _game.GameStatus).Playing && !card.isFlipped) {
                    (0, _game.flipCard)(gameState, card.id); // Flip the selected card
...
function updateGame(gameState) {
    ...
    if (gameState.status === (0, _game.GameStatus).Won) alert("Congratulations! You've matched all pairs!"); // Win message
    else if (gameState.status === (0, _game.GameStatus).Lost) alert("Game Over! You've used all attempts."); // Loss message
}
// Initial setup function for starting and resetting the game
function setupGame() {
    const gameState = (0, _game.initializeGame)(); // Initialize a new game state
    renderGame(gameState); // Render the initial game UI
    // Attach click event listener to the restart button
    const restartButton = document.getElementById("restartButton");
    if (restartButton) restartButton.addEventListener("click", ()=>{
        (0, _game.resetGame)(gameState); // Reset the game state
...
},{"./Game":"iZsgX"}],"iZsgX":[function(require,module,exports) {
...
parcelHelpers.export(exports, "flipCard", ()=>flipCard);
...
function initializeGame() {
    ...
    const themes = [
        (0, _card.CardTheme).Light,
        (0, _card.CardTheme).Dark,
        (0, _card.CardTheme).Blue
    ];
    const cards = symbols.map((symbol, index)=>(0, _card.createCard)(index + 1, symbol, themes[index % themes.length]));
    return {
...
function shuffleCards(cards) {
    return cards.map((card)=>({
            ...card,
            sortKey: Math.random()
        })).sort((a, b)=>a.sortKey - b.sortKey).map(({ sortKey, ...card })=>card);
}
function flipCard(gameState, cardId) {
    ...
    if (card && !card.isMatched && !card.isFlipped) {
        ...
        if (gameState.flippedCards.length === 2) checkForMatch(gameState);
    }
}
...
},{"./Card":"dopFD","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"dopFD":[function(require,module,exports) {
...
var CardTheme;
(function(CardTheme) {
    CardTheme["Light"] = "light";
    CardTheme["Dark"] = "dark";
    CardTheme["Blue"] = "blue";
})(CardTheme || (CardTheme = {}));
const createCard = (id, symbol, theme = "light")=>{
    return {
        id,
        symbol,
        isMatched: false,
...
},{}]},["4JiOW","8tyFx"], "8tyFx", "parcelRequiref3a7")
